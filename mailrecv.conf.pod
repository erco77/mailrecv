...
SMTP command, which is normally the failure response to EHLO, which
...
This is mainly for spam prevention, to prevent the remote from
...
The time also includes how long it takes DNS lookups on remote's IP.

<val> is the maximum number of seconds allowed for the remote connection
to complete and close before the connection timer expires, forcing a
disconnect. Value must be greater than 1.
...
=item B<limit.smtp_data_size> <val> <ecode> <error_message_text>
...
<val> is the maximum number of message-content bytes. If exceeded, message
reception stops, the configured error response is sent, and the SMTP session
is terminated without delivering the message. Value must be greater than 1.
...
<val> is the maximum number of RCPT TO commands allowed. If exceeded, the
configured error response is sent and the SMTP session is terminated.
...
=item B<limit.fcrdns_check> <on|off> <ecode> <error_message>

Enables Forward-Confirmed Reverse DNS (FCRDNS) checking. Mailrecv performs a
reverse DNS lookup of the remote IP address, then resolves the resulting
hostname forward and verifies that at least one returned address matches the
remote IP.

<on|off> enables or disables the check. The default is C<on>. When enabled,
a failed lookup or address mismatch causes mailrecv to disconnect the remote
with the specified <ecode> and <error_message>.

FCRDNS checking is skipped when mailrecv is running interactively on a
terminal. DNS lookup time counts toward B<limit.connection_secs>.

<ecode> must be a 3 digit RFC 822 compliant "reply code". 450 is recommended
(reply code for transient errors), because DNS lookups can fail temporarily.
This allows a valid sending server to try again later.

An FCRDNS failure can indicate missing or inconsistent DNS configuration, but
a successful check does not authenticate the sending server or guarantee that
it is trustworthy.

This example shows the default:

    limit.fcrdns_check on  450 Failed FcrDNS, try again later
                        --  ----------------------------------
                        Value Error message
...
=item B<deliver> [B<allowgroup> <+name>] B<rcpt_to> <email_address> B<append> <filename>

Configures inbound delivery for <email_address> by appending the email
contents to <filename>.

If B<allowgroup> is omitted, delivery is allowed from any remote that passed
the global B<allow remotehost> checks. If B<allowgroup> is specified, the
remote must also match one of the patterns in the named group.

Examples:

    deliver rcpt_to archive@example.com append /var/mail/archive
    deliver allowgroup +trusted rcpt_to private@example.com append /var/mail/private
...
<filename> via flock(LOCK_EX), ensuring messages aren't interwoven.

=item B<deliver> [B<allowgroup> <+name>] B<rcpt_to> <email_address> B<pipe> <command> [args...]

Configures inbound delivery for <email_address> by piping the email contents
to the standard input of <command> with any optional arguments.
...
Examples:
    ...
    deliver rcpt_to archive@example.com pipe /usr/local/bin/archive-mail
    deliver allowgroup +trusted rcpt_to alerts@example.com pipe /usr/local/bin/process-alert
...
Restricts remote host connections to IPV4, IPV6, or FQDN that match <regex>.

Multiple "allow remotehost" commands can be specified to allow more than
one remotehost <regex>.

If no B<allow remotehost> directives are configured, all remote hosts pass
the global allow-list check.

This global check applies to every delivery directive, including directives
specifying an B<allowgroup> directive, which adds a recipient-specific restriction, and
does not bypass or replace the global allow-list. Example:

    allow remotehost ^1\.2\.3\.4$
    deliver rcpt_to fred@ourdomain.com append /home/fred/emails.txt

Multiple B<allow remotehost> directives may be specified. A remote matching
any one of the patterns passes the global check:
...
B<1)> Put the following in /etc/mailrecv.conf
    ...
    # Ensure all other RCPT TO email delivery attempts fail with this error
    error rcpt_to .* 550 No one here by that name.
